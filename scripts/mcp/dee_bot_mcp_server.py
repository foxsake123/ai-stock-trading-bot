...
class DeeBot:
    TARGETS = {"AAPL": 0.10, "JNJ": 0.10, "PG": 0.08, "KO": 0.08, "PFE": 0.07, "VZ": 0.07, "MRK": 0.07, "CSCO": 0.07, "SO": 0.06, "MDT": 0.06}
    STOP_LOSS = 0.11
...
server = Server("dee-bot") if MCP_OK else None
...
if server:
    ...
    async def list_tools():
        return [
            ...
            Tool(name="run_dee_bot", description="Run DEE-BOT", inputSchema={"type": "object", "properties": {"user_id": {"type": "string"}, "dry_run": {"type": "boolean", "default": True}, "confirm_live": {"type": "boolean", "default": False}}, "required": ["user_id"]}),
    ...
    async def call_tool(name, args):
        ...
        try:
            if name == "connect_account":
                ...
                await audit.write("run", c.user_id, result.get("ok", False), {"pnl": result.get("pnl", 0)})
...
async def main():
    print("DEE-BOT MCP Server v2.0", file=sys.stderr)
    if not init():
